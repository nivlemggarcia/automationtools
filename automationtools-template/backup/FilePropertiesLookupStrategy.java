...
import com.automationtools.template.AbstractPropertyLookupStrategy;
...
public class FilePropertiesLookupStrategy extends AbstractPropertyLookupStrategy {
	...
	public Properties get(Key templatekey) {
		...
		log.debug("Property file found for {}", templatekey.getRaw());
		readLock().lock();
		try {
			...
			throw new FailedToLoadResourceException(templatekey.getRaw(), e);
		} finally {
			readLock().unlock();
		}
	}
	...
	public boolean update(Key templatekey, Properties newproperties) throws Exception {
		...
		try {
			if(propertyFile == null) {
				...
				propertyFile = new File(sourceDirectory, templatekey.getRaw() + _PROPERTY_FILE_SUFFIX);
	...
	public void setSourceDirectory(File sourceDirectory) {
		Assert.notNull(sourceDirectory, "Source directory cannot be null");
